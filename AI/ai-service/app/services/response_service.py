...
class ResponseService:
    ...
    def get_response(self, template_key: str, language: str, params: Dict[str, Any] = None) -> str:
        """템플릿 키와 언어에 맞는 응답 생성"""
        # 언어 기본값 설정
        lang = language if language in self.templates else Language.KO
        ...
        if not template:
            ...
            if not template:
                template = self.templates.get(Language.KO, {}).get(template_key, "")
    ...
    def generate_order_reply(self, menus: List[Dict[str, Any]], language: str, status: ResponseStatus) -> str:
        ...
        menu = menus[0]
        menu_name = menu.get("name") if language == Language.KO else menu.get("name_en", menu.get("name"))
        ...
        # 3. 필수 옵션 누락 상태
            ...
            if missing_options:
                ...
                option_name = option.get("option_name") if language == Language.KO else option.get("option_name_en", option.get("option_name"))
