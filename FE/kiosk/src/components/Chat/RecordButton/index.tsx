...
interface Props {
  ...
  recordingMethod: 'pitch' | 'hold';
}

export interface RecordButtonRef {
  updatePreBuffer: (buffer: Float32Array[]) => void;
}

const RecordButton = forwardRef<RecordButtonRef, Props>(
  ({ onRecognitionResult, setRecordingMethod, recordingMethod }, ref) => {
    // 상태 관리
    ...
    const handlePressStart = async () => {
      setError(null);
      setRecordingMethod('hold');
      await startRecording();
    };

    // 녹음 중지
    const handlePressEnd = async () => {
      if (isRecording && recordingMethod === 'hold') {
        try {
    ...
    useImperativeHandle(ref, () => ({
      updatePreBuffer: (buffer: Float32Array[]) => {
        preBufferRef.current = buffer;
      },
    }));
